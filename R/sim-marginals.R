# Purpose: Resolve a target marginal response distribution into the per-item GRM
#          thresholds the attentive generator (R/sim-attentive.R) thresholds its
#          latent values against. A named preset, an explicit category-probability
#          vector, or a per-spec strength is converted EXACTLY to thresholds via
#          tau_k = qnorm(cumsum(p)), so under normal traits the categorised
#          marginal equals the target pmf in closed form (the marginals oracle).
# Args:    See per-function signatures.
# Returns: pmf vectors or threshold lists; raise cier_error_input on bad input.
# Invariants:
#   - Pure (no I/O, no state). Mutually exclusive with raw `thresholds`.
#   - Every pmf is strictly positive and is re-normalised to sum exactly to 1
#     before the quantile transform, so the interior cumulative sums stay in
#     (0, 1) and the thresholds are finite and strictly increasing (a raw sum a
#     few ulp above 1 would otherwise push qnorm to +Inf / NaN).

# The five named marginal-shape presets, as a strictly-positive pmf of length k
# (Markus, 2026-06-11). `strength` (>= 1, default 1.5, per-spec overridable)
# steepens the geometric skew and is ignored for the non-skewed presets.
#   - uniform: flat 1 / k.
#   - peaked: symmetric triangular w_j = min(j, k + 1 - j); k = 5 -> (1,2,3,2,1)/9.
#   - skewed_left: geometric w_j = strength^(j - 1), mass on the HIGH (agreement)
#     categories; skewed_right is its mirror (mass on the low categories).
#   - bimodal: deep reflected-triangular U w_j = (max(tri) + 1) - tri_j; k = 5 ->
#     (3,2,1,2,3)/11.
...
# +/- Inf and break strict monotonicity; a negative entry is not a probability).
...
# Validate the names of a scale-named `marginals` list: every entry named (no
# silent positional fallback for a half-named list), no repeated scale (no silent
# first-wins), and exactly the battery's scales covered.
...
# Expand the `marginals` argument into a per-item list of specs (length p).
# Disambiguation: NULL -> peaked everywhere; a single preset string or pmf vector
# or a `preset`-keyed list -> one spec for every item; a fully scale-named list ->
# per scale (names must cover exactly the scales); a fully unnamed length-p list ->
# per item. A partially-named list is rejected (never reinterpreted positionally).
...
# validated list from check_items_simulate (carries `scale` and `categories`).
...
# Convert a per-item pmf list to per-item GRM thresholds: tau_k = qnorm(cumsum(p))
# for k = 1..K-1 (the final cut qnorm(1) = Inf is dropped). The pmf is normalised
# to sum exactly to 1 first, so a validated pmf whose raw sum is a few ulp above 1
# cannot push an interior cumulative sum to 1 and send qnorm to +Inf / NaN; with
# strictly-positive mass the thresholds are then finite and strictly increasing.
