# Careless-pattern block producers for cier_simulate(). Each fills an
# n_rows x length(mins) integer block with one C/IER pattern in raw coding (min_j..max_j);
# R/sim-plan.R writes it into a respondent's careless span. `call` is threaded so a bad
# parameter is attributed to the public caller. Sequential patterns (diagonal, alternating,
# markov) walk one K_min = min(categories) and offset per item by min_j, so values stay in
# range on a heterogeneous battery. speeder is a pattern NAME (attentive content, careless
# times in a later layer), not a content producer.

# ---- Shared position helpers ------------------------------------------------

# Round half UP (floor(x + 0.5)): the position convention (a 4-option midpoint is the
# upper middle, 3), NOT base round()'s round-half-to-even.
sim_round_half_up <- function(x) {
  floor(x + 0.5)
}

# The n_rows x p value grid at relative position q (length n_rows): per cell
# round_half_up(min_j + q_i * (max_j - min_j)).
...
# Resolve a per-row scalar parameter: NULL draws one per row via `draw`; otherwise a
# scalar or length-n_rows vector in [lo, hi] (whole when `whole`), recycled to n_rows.
...
# Resolve the straightline `value` against the common achievable range [max(min),
# min(max)] -- the values a single number can take on every item. NULL draws one whole
# number per row from it; an explicit value must be a whole number in it (out-of-range is
# a typed error, not silently clipped).
...
# The n_rows x p switched-state grid for the straightline toggle: column 1 never
# switched; each later column flips when an independent Bernoulli toggle fires, so the
# held position PERSISTS between switches (cumulative parity, not a per-column coin).
# switch_prob = 1 alternates F, T, F, T ...; 0 never switches.
...
# straightline: each row holds one relative position (anchor = "position", q in [0, 1]
# mapped per item) or one constant value (anchor = "value"). switch_prob toggles to the
# mirrored response (q -> 1 - q, value -> (min + max) - v) and holds until the next
# switch, producing longstring runs.
...
# (triangle-wave) ramp that climbs to K and back. K = K_min; position offset to
# min_j + pos - 1. start defaults to a random position.
sim_block_diagonal <- function(n_rows, mins, maxs, params = list(),
  ...
  # The walk repeats every `period`: K_min for the cyclic ramp, 2*(K_min - 1) for the bounce
  # triangle wave. A step that is a multiple of the period freezes the position (constant
  # column), so it is rejected.
...
# Default alternating positions: `period` points spread over 1..K_min (the high/low
# seesaw c(1, K) at period 2), or a cyclic 1..K_min run when period > K_min.
...
# period, offset to min_j + pos - 1. period d peaks the autocorrelation at lag d.
...
# Default transition over K positions: 0.6 of staying, the rest spread uniformly -- a
# predictable but non-degenerate chain (the matched generator for cier_lazr).
...
# Stationary distribution of a transition matrix (left eigenvector for eigenvalue 1),
# falling back to uniform when it is not uniquely real-positive.
...
# markov: a first-order chain over positions 1..K_min governed by `transition` (the
# first item drawn from `initial`), offset to min_j + state - 1.
...
# The eight recognised pattern NAMES (the weights allowlist). speeder's content is
# attentive (carelessness is timing only), so it is in the allowlist but not the content
# registry below.
...
# The accepted per-pattern knob names (the pattern_params allowlist), one entry per pattern.
# Mirrors each producer's `params$...` reads, so a typo'd knob is a typed error rather than a
# silently-ignored default. random and speeder take no knobs.
sim_pattern_knob_names <- function() {
  list(
    random       = character(0L),
    straightline = c("anchor", "q", "value", "switch_prob"),
    midpoint     = "jitter",
    extreme      = "p_high",
    diagonal     = c("step", "start", "bounce"),
    alternating  = c("period", "values", "start_offset"),
    markov       = c("transition", "initial"),
    speeder      = character(0L)
  )
}
