...
#
# The slow tier re-fetches the external datasets (fetch-only; never bundled)
# and recomputes every cier cell of inst/extdata/published-results.csv with
# the same conventions and seeds the vignette uses. This is a deliberate
# white-box check: it pins CSV<->code agreement; faithfulness to the papers
# is pinned by the frozen transcription and the allowed-differs list in the
# fast tier (see tests/reference/TOLERANCES.md, "Published-results
# reproduction").
...
# Skip (never fail) when an upstream file is missing or has silently changed:
# a re-uploaded dataset must be re-inspected, not silently compared.
...
# Digest of the paper-side (transcription) columns only: the cier-side columns
# are free to change on a re-knit, the transcription is not.
...
# Rank (Mann-Whitney) AUC of `score` for label 1 over label 0; the score must
# already be oriented careless-high. Non-finite scores and NA labels drop.
...
# Score every respondent on a fixed pair list (Meade & Craig per-person
# correlation across stacked pairs) -- used for the careful-reference
# synonym / antonym companion cells, where pair discovery and scoring run on
# different row sets so the wrapper's whole-sample pairing does not apply.
...
pubres_battery_values <- function(w) {
  pooled <- function(fn) {
    ...
    for (k in seq_along(unique(dom))) {
      ...
      # only typed cier errors (e.g. a backend span limit) abstain a domain;
  ...
  # no tryCatch on the pairing wrappers: a zero-pair sample must ABSTAIN
  # (NA values), never error -- an error here is a real defect and must
  # fail the test loudly
  rpr <- cier_personal_reliability(w$m, w$items, seed = pubres_seed)$value
  vals <- list()
  vals$RPR <- list(v = rpr, dir = "upper")
  vals$MD <- list(v = cier_mahalanobis(w$m)$value, dir = "upper")
...
# Careful-reference companion scores (Studies 4/5): pairs discovered on the
# careful conditions' second wave, scored on every respondent of both waves.
...
# Recomputed Table 9 cells: PR = even-odd consistency (the paper's definition),
# RPR at 25 / 50 / 100 resamples, each on the 6- / 16- / 23-facet pools.
...
# The authors' scale-blocked item order for even-odd consistency (their
# analysis script 06_online_trad: items_hh / _em / _ex / _ag / _co / _op).
...
# The authors' binarisation rules (their OSF scripts 03 + 06), one tiny
# function per rule so the fast tier can pin each boundary deterministically.
# IRV: the paper computed both tails and kept the better-performing side; on
# this data that is the LOWER tail (instructed-inattentive respondents
# straightline), which reproduces all five Table-3 metrics -- the upper tail
# (the only variant left uncommented in their script) does not.
...
# Replicate the authors' evaluation loop: 1,000 random test samples of 162
# regular + 18 careless respondents; per draw, each index is computed within
# the draw and binarised with the authors' published rule; the reported cell
# is the mean metric across draws.
