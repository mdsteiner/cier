# Fast-tier contracts for inst/extdata/published-results.csv -- the committed table
# behind the published-results vignette. No network, no recomputation: this file pins
# (1) the schema, (2) the transcribed paper values (digest + verbatim spot checks
# against the cited source tables), (3) the row inventory, (4) the status /
# empty-cell / explanation contracts, and (5) a few coarse direction floors that a
# sign or orientation flip cannot survive. The slow tier
# (test-published-results-reproduction.R) recomputes the cier cells from fetched data.

pubres_statuses <- c("matches", "close", "differs", "abstains",
                     "not_implemented", "not_evaluated")

# Cells allowed to carry status "differs" -- each a pre-registered, explained
# divergence (see the vignette prose). A new divergence anywhere else must be
# diagnosed and added here deliberately, with its explanation, before the suite
# passes. The cap bounds how far even an allowed divergence may go: it converts the
# allowed-differs pocket from "any value" to "the explained mechanism's plausible
# size", so a sign or orientation flip cannot hide inside it.
pubres_allowed_differs <- function(tab) {
  ...
  # whole-sample synonym pairing on Studies 4/5 finds only a few attenuated pairs on
  # the careless wave: scored, but well below the paper's careful-derived pairing
  ...
  # the Bruhlmann consistency counts inherit the authors' correction of one mis-keyed
  # openness item (their analysis script; the vignette demonstrates that replicating
  # it reproduces their counts exactly)
...
test_that("the not-implemented set is pinned exactly", {
  ...
  expect_setequal(unique(ni$index[ni$paper == "schroeders_2022"]),
                  c("Zh", "GBM (responses)", "GBM (response times)",
                    "GBM (responses + times)"))
  # the response-time row stays empty for a different reason than the rest: the
  # authors flagged nobody by visual inspection and published no numeric cutoff, so
  # there is no rule to reproduce (cier_total_time itself shipped)
...
test_that("status, empty-cell, delta, and explanation contracts hold", {
  ...
  # the abstention inventory is pinned exactly, from the measured data: on Studies
  # 1/2 the single contaminated wave yields no pairs at critical_r = .60 for either
  # pairing index; on Studies 4/5 the careless second wave yields no ANTONYM pairs
  # (synonyms keep a handful and score, as capped differs cells). Nothing else may
  # abstain -- a silently relaxed critical_r must surface here as a deliberate change.
  ...
  # not_evaluated means the paper has no value -- cier shows none either (any
  # cier-side observation lives in the note, not the value column)
  expect_true(all(is.na(tab$cier_value[tab$status == "not_evaluated"])))

  # every divergence and every abstention carries a written explanation, and
  # divergences appear only in the pre-registered allowed list, bounded by each
  # mechanism's cap
...
test_that("values are in range and the reproduction is not vacuous", {
  ...
  # coarse direction floors, one per family with an allowed-differs pocket included:
  # an orientation flip, label mis-slicing, or wrong-group construction cannot
  # survive these
...
test_that("the shared statistics pin their definitions on fixtures", {
  ...
  # sen95: the cutoff is the CAREFUL group's 95th percentile, exceeded STRICTLY.
  # careful = 1..20 makes the type-7 cutoff exactly 19.05; a careless score equal to
  # it must not count (>= would give 1 here).
  s <- c(1:20, 19.05, 25)
  l <- c(rep(0L, 20), 1L, 1L)
  expect_identical(pubres_sen95(s, l), 0.5)
  # and the quantile must come from the careful group: with all careless scores far
  # above every careful score, sensitivity is exactly 1 (a careless-group cutoff
  # would drop it to 0.25)
  ...
  # with two 1s and 2..10 the type-7 cutoff is exactly 1, so both 1s flag (strict <
  # would flag none; the upper-tail mutant flags neither)
  v <- c(1, 1, 2:10)
