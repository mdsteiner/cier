...
#
# Trust model: the independent oracle (ref_person_total) re-derives each respondent's
# correlation with the whole-sample item means via a per-row stats::cor() loop and
# never calls the production kernel (which evaluates the whole battery with vectorised
# masked sums); a separate hand-computed fixture pins exact +/-1 values sharing no
# masked-sum machinery. The cross-package check pins parity with PerFit::r.pbis at
# 1e-4 (PerFit rounds its output to 4 dp; the correlation itself is exact). The
# property / regression block targets each documented mutant (item-rest instead of
...
# Hand-computed fixture. The column means are (1.75, 2.25, 2.75, 3.25) -- a perfectly
# linear increasing sequence -- so an increasing row correlates +1 with them and a
# decreasing row -1. Values are c(1, 1, -1, 1), worked by hand and confirmed against
# the oracle, with no masked-sum machinery in the expectation: a gross formula error
# cannot survive this.
...
# Independent item-REST (leave-one-out) reference, used only to prove the production
# statistic is item-TOTAL and not item-rest. Hand-rolled; never calls the kernel.
...
test_that("cier_person_total returns the pinned cier_index schema", {
  expect_cier_index_schema(suppressWarnings(cier_person_total(hand_fixture())),
                           "cier_person_total", "lower", 4L)
})
...
test_that("cier_person_total matches PerFit::r.pbis (cross-package, 4-dp)", {
  ...
  # implementation. The person-total correlation is invariant to an additive shift,
  # so our raw 1/2 coding agrees with PerFit's zero-based 0/1 coding; the only
  # divergence is PerFit's 4-dp output rounding (observed ~4.6e-5).
...
test_that("the statistic is item-TOTAL, not item-rest (leave-one-out)", {
  ...
  # (~0.66 here). We must match the item-total oracle to 1e-12 and stay far from the
  # item-rest values; an item-rest mutant would invert both assertions.
...
test_that("the wrapper is matrix-only: no item-metadata channel exists", {
  # Pins keying-insensitivity and whole-sample (not scale-level) means structurally:
  # no items / scale / reverse_keyed / max argument, so neither a reverse-keying nor
  # a scale-means mutant can be wired in. The whole-sample colMeans definition is
  # enforced by the oracle parity.
...
test_that("direction is lower: a careless (low-correlation) row flags, a consistent one does not", {
  # 20 rows tracking an increasing item profile plus one decreasing
  # (anti-correlated) row. The anti row is the global minimum correlation (most
  # careless), so it flags at the lower-tail cutoff and the most consistent row does
  # not. A flag-upper mutant would invert both.
  withr::with_seed(11L, {
...
test_that("a respondent with exactly two answered items abstains; three scores", {
  # cor() of two points is always +/-1, so without the k < 3 guard a 2-item row would
  # score a spurious +/-1. The guard forces NA at k = 2 and scores at k = 3 (the
  # boundary): a k < 2 mutant scores row 1, a k < 4 mutant abstains row 2.
...
test_that("a NON-INTEGER straightliner abstains exactly, with no leaked base-R warning", {
  ...
  # decimal constant (POMP / rescaled / averaged scores are documented numeric input)
  # lands it a few ulp on EITHER side of zero: tiny-negative used to leak an untyped
  # base-R "NaNs produced" warning through sqrt(); tiny-positive used to leak a
  # spurious finite ~1e-7 into the percentile pool and flag count. The kernel now
  # detects the constant row exactly (masked min == max), so every constant abstains
  # silently regardless of which way the cancellation fell. 2.597092 / 1.663422
  # landed tiny-negative / tiny-positive at k = 12 under the old kernel; 0.3 and 0.05
  # are realistic decimal codings.
  for (const in c(2.597092, 1.663422, 0.3, 0.05)) {
    # 25 rows keep the cutoff resolvable (>= 20 scored), so expect_no_warning
    # isolates the kernel's silence on the constant row from the cutoff layer.
...
test_that("a flat item-mean profile abstains (zero variance on the item side)", {
  # Distinct from the k < 3 path: this circulant answers every item (k = 4) and every
  # row is non-constant, but the column means are all equal (2.5), so the correlation
  # has zero variance on the item-mean side and must be NA. A mutant guarding only
  # the respondent side (constant rows) would return NaN here.
...
test_that("a column answered by nobody yields finite scores (no NaN) and matches the oracle", {
  # An all-NA column gives a 0/0 = NaN item mean; the kernel forces it finite (0) so
  # it drops out of the masked products instead of poisoning every row. A mutant
  # skipping that guard turns every value NaN.
...
test_that("a two-column matrix abstains for every row and flags nobody", {
  # Each row has at most two answered items -> all abstain -> the percentile cutoff
  # has no finite values: it warns and returns NA, and an NA cutoff flags no one.
  expect_warning(
...
test_that("an absolute cutoff overrides the percentile and flags via direction", {
  # hand_fixture values are c(1, 1, -1, 1); lower direction => value <= 0 flags, so
  # only the anti-correlated (-1) respondent is flagged.
...
test_that("a respondent exactly at the cutoff is flagged (<= ties, not <)", {
  ...
  # cutoff flags. Setting the cutoff to one respondent's own score: a strict-less (<)
  # mutant would leave it unflagged.
  x <- rand_matrix(n = 30L, p = 8L, seed = 7L)
