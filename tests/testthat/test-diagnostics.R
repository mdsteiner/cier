...
# oracle; see tests/reference/ref-poisson-binomial-enumeration.R and
# tests/reference/TOLERANCES.md.

source(test_path("..", "reference", "ref-poisson-binomial-enumeration.R"))

# A logical n x m flag matrix whose columns fire at distinct marginal rates,
# so the Poisson-binomial baseline (unequal p) is genuinely exercised.
...
test_that("per_vote reports observed rates; null_rate drives excess + informative", {
  ...
  # No null supplied: every column is tautological (an empirical percentile
  # flags fpr by construction), so nothing is informative.
  ...
  # The vote identifier falls back to vote1..voteM when the matrix is unnamed,
  # and carries the column names through when present.
  ...
  # A null nominal on the third (null-referenced) vote only: its excess over
  # the calibrated null is informative; the percentile columns stay NA.
