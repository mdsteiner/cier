# Tests for cier_flagged_cases() -- the accessor that turns a cier_index or a
# cier_screen into the row positions of the flagged respondents.
#
# It is a pure extractor (no statistics), so the contract is: the cier_index
# method returns which(flagged) (NA-safe, 1-based, ascending); the cier_screen
# method thresholds the COLLAPSED votes (rowSums(votes) >= min_votes), so it
# agrees exactly with the agreement diagnostic (length == the ">= k votes" count)
# and never double-counts the consistency construct. The decisive test is the
# collapse one: a "counts raw per-index flags" implementation flags a respondent
# hit by both even-odd and PR at min_votes = 2, whereas the collapsed count is 1.
...
test_that("the cier_index method returns flagged positions, NA-safe", {
  ...
  # abstaining respondent is never returned; FALSE rows are dropped too.
  ...
  # min_votes is a cier_screen-only argument; for a single index it is inert
  # (absorbed by `...`), not silently used as a threshold.
  expect_identical(cier_flagged_cases(idx, min_votes = 99), c(1L, 4L))
})
...
test_that("flagged-case count matches the agreement table, on real multi-vote data", {
  ...
  # is what makes the per-level pins bite: which(>= 1), which(>= 2), which(>= 3) are
  # all different non-empty sets, so an "ignores min_votes" mutant (returns the
  ...
  # set strictly shrinks as the threshold rises. This is the decisive guard against
  # an "always integer(0) above 1" or a "min_votes-ignoring" implementation.
...
test_that("min_votes counts collapsed votes, not raw per-index flags", {
  ...
  # Each such respondent then carries TWO raw flags but only ONE construct vote,
  # so min_votes = 2 must return nobody. A raw-flag implementation returns them all.
...
test_that("output is unnamed positional integers even with row-named input", {
  # A data.frame input with rownames makes an index's `flagged` named; the
  # accessor must still return BARE positional indices (the documented contract),
  # and the index and screen methods must agree on that.
