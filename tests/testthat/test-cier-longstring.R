...
#
# Trust model: oracle ref_longstring re-derives the max-run statistic via a
# hand-rolled loop, never calling the kernel; the cross-package check pins
# bytewise parity with careless::longstring (tolerance 0); the property /
# regression block targets each documented mutant.

source(test_path("..", "reference", "ref-longstring-johnson2005.R"))

# Present-data fixture (no all-NA row), so the value is defined for every row and
# the oracle agrees row for row.
...
test_that("cier_longstring returns the pinned cier_index schema", {
  out <- cier_longstring(present_fixture())
  expect_cier_index_schema(out, "cier_longstring", "upper", 4L)
  # present_fixture values 1, 8, 1, 4; at default cutoff ceiling(0.5 * 8) = 4,
  # rows 2 and 4 flag.
...
test_that("NA breaks runs and is never treated as equal to NA (no merge mutant)", {
  # rle runs (1),(NA),(NA),(1) all length 1 -> max 1. An NA == NA merge mutant
  # returns 2. (p = 4 trips the short-battery warning, incidental here -> muffled.)
  out <- suppressWarnings(cier_longstring(matrix(c(1, NA, NA, 1), nrow = 1L)))
...
test_that("an abstaining row in the middle keeps value/flagged row-aligned", {
  # Guards a row-indexing / apply mutant that mis-aligns value and flagged when an
  # all-NA row is not last. p = 6, cutoff 3.
  x <- matrix(
...
test_that("a single-column matrix yields longstring 1 for every row", {
  # p = 1 -> default cutoff 1 -> short-battery warning, incidental here -> muffled.
  out <- suppressWarnings(cier_longstring(matrix(c(3, 4, 5), ncol = 1L)))
  expect_identical(out$value, c(1, 1, 1))
})

test_that("single-row and constant matrices behave", {
  # The 4-item row trips the short-battery default-cutoff warning -> muffled.
  out4 <- suppressWarnings(cier_longstring(matrix(c(1, 1, 2, 3), nrow = 1L)))
  expect_identical(out4$value, 2)
...
# ---- short-battery default-cutoff warning -----------------------------------

test_that("the default cutoff <= 2 on a short battery warns", {
  # p = 4 -> default ceiling(0.5 * 4) = 2 <= 2 -> short-battery warning. Score and
  # flags unchanged (no abstention); only a typed warning is added.
  x <- matrix(c(3, 3, 3, 3,        # value 4 -> flag at cutoff 2
                1, 2, 3, 4,        # value 1
                5, 5, 1, 2,        # value 2 -> flag at cutoff 2
                1, 2, 1, 2),       # value 1
              nrow = 4L, byrow = TRUE)
  expect_warning(out <- cier_longstring(x), class = "cier_warning_short_battery")
  expect_identical(out$cutoff, 2)
  expect_equal(out$value, c(4, 1, 2, 1))
  expect_identical(out$flagged, c(TRUE, FALSE, TRUE, FALSE))
  w <- testthat::capture_warnings(cier_longstring(x))
  expect_length(w, 1L)
  expect_match(w, "short batter", ignore.case = TRUE)
})

test_that("p = 3 default warns; p = 5 (cutoff 3) is silent (boundary)", {
  # ceiling(0.5 * p): p = 3 -> 2 (warn), p = 4 -> 2 (warn), p = 5 -> 3 (silent).
  expect_warning(cier_longstring(matrix(c(1, 2, 3, 3, 3, 3),
                                        nrow = 2L, byrow = TRUE)),
                 class = "cier_warning_short_battery")
  expect_no_warning(cier_longstring(matrix(rep(c(1, 2, 3, 4, 5), 2L),
                                           nrow = 2L, byrow = TRUE)))
})

test_that("an explicit frac / cutoff suppresses the short-battery warning", {
  # DEFAULT-path guard only: a user who set the cutoff deliberately gets silence.
  x <- matrix(c(3, 3, 3, 3, 1, 2, 3, 4), nrow = 2L, byrow = TRUE)   # p = 4
  expect_no_warning(cier_longstring(x, cutoff = 2))
  expect_no_warning(cier_longstring(x, frac = 0.5))
})

test_that("a long-battery default does not warn (negative case)", {
  expect_no_warning(cier_longstring(matrix(rep(3, 20L), nrow = 2L)))  # p=10 cutoff 5
})
