...
# find_item_pairs() (the pairing the index scores) and, cross-package, against
# careless:::psychsyn_critval().

# Independent lower-triangle pair table (item_i > item_j), optionally filtered to a
# critical_r tail, sorted like the helper. Hand-rolled; no call to production code.
...
# latent factor f (mutually positive = synonyms), cols 4-5 on -f (negatively
# correlated with 1-3 = antonyms, several distinct r values), so the antonym tail
# has > 1 pair to pin the ascending sort and the min strongest_r.
...
test_that("the filtered pair set equals the production find_item_pairs() set", {
  # The lister must surface exactly the pairs the index scores at the same critical_r
  # -- otherwise the diagnostic would mislead the cutoff choice.
...
test_that("the critical_r threshold is strict (> not >=) at an exact pair r", {
  ...
  # off-by-one (>= vs >) survives expect_setequal. Setting critical_r to the exact
  # correlation of a real pair: a strict > excludes that pair, a >= mutant keeps it.
  # Pinned against the production find_item_pairs at the same boundary.
...
test_that("a constant (zero-variance) column forms no pair and leaves r finite", {
  ...
  # never leak an NA r (nor, in the sweep, poison strongest_r).
...
test_that("antonym output with several pairs is sorted by ASCending r (most negative first)", {
  ...
  # multiple antonym pairs of differing strength. A descending / no-sort mutant on the
  # negative tail survives a 1-row frame but fails here.
  x <- ant_matrix_d()
  out <- cier_synonym_pairs(x, critical_r = 0.6, antonym = TRUE)
...
# Map careless's name-keyed pairs (var1, var2 factors) to "i-j" column-index keys,
# larger index first, to compare against cier's item_i / item_j.
...
test_that("cier_synonym_pairs matches careless::psychsyn_critval (synonyms)", {
  # GENUINE cross-package parity: careless::psychsyn_critval() returns the full p x p
  # long frame (var1, var2, cor) with upper triangle + diagonal NA; its non-NA rows
  # are the lower-triangle pairs sorted by descending correlation -- exactly what
  # cier_synonym_pairs(critical_r = NULL) produces. Same pairs, correlations, order.
