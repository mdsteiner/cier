# Tests for the attention-check injection (Slice 26) -- the direct half of
# cier_simulate(). Checks are generated OUTSIDE the response matrix, shaped
# exactly for the shipped cier_attention(checks, pass) contract; failure is
# probabilistic on BOTH sides (careless rows fail at p_fail_careless = 0.75,
# attentive at p_fail_attentive = 0.05 -- documented conventions, not
# estimates), so neither group separates perfectly.
#
# Trust model: oracle-only. The independent oracle (ref-sim-times.R)
# re-derives the injection by hand with a coordinated draw order; the
# deterministic p_fail edges are pinned THROUGH the shipped cier_attention.
