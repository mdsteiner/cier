# Tests for the attentive graded-response model (GRM) generator (Slice 24) --
# the careful-respondent layer of cier_simulate(). Pure internal kernels: no
# public cier_simulate() and no S3 object yet (Slice 26), so these call the
# kernels directly in the package namespace.
#
# Trust model: oracle-only (the simulator is a generator, not an index, with no
# CRAN partner). The independent oracle (ref-sim-attentive-grm.R) re-derives the
# preset pmf, the categorisation, and the implied marginal from scratch and never
# calls production. Fast tier: exact closed-form marginals (pnorm of the resolved
# thresholds == target pmf at 1e-12, no RNG), categorisation tol 0, the API /
# validator / shape edges, deterministic structure, seed reproducibility. Slow
# tier (skip_if_slow + skip_on_cran): the large-n marginal-frequency oracle
...
# Build a raw items data.frame in the lean min/max/scale schema (what
# sim_attentive() validates internally). `max` / `min` / `reverse_keyed` recycle.
...
# The full preset set, in one place (the production switch and the oracle each
# enumerate it independently).
...
test_that("a preset-keyed list is a single spec, not a per-scale map, across scales", {
  # Disambiguation pin: a list with a `preset` element is ONE spec applied to
  # every item, even on a MULTI-scale battery (a per-scale misread would demand
  # E/N keys and error). A bare list(preset=) with no strength uses the default.
  it <- vit(c("E", "N"), max = 5)
...
test_that("a partially-named list is rejected, never read positionally", {
  # A forgotten name must NOT silently degrade to per-item positional mapping.
  it <- vit(c("E", "N"), max = 5)
...
test_that("a pmf whose sum is a few ulp above 1 still yields finite thresholds", {
  # The validator's 1e-8 sum tolerance admits c(0.25, 0.75, 1e-9); normalisation
  # in the threshold transform keeps the cuts finite and the call non-erroring
  # (no qnorm(>1) = NaN crash, no qnorm(1) = Inf unreachable category).
...
test_that("marginals and thresholds resolve end-to-end through sim_attentive", {
  # Routes a NON-NULL marginals= AND a non-NULL thresholds= through the full
  # chain (fast tier). A marginals/thresholds argument swap at the dispatch
  # would either error here or land the wrong thresholds in $metadata.
...
test_that("user loadings change the generated factor structure (fast)", {
  # Pins that loadings actually flow through: a mutant substituting the 0.7
  # defaults could not separate these. Higher loadings -> stronger within-scale
  # correlation, under the same seed.
  it <- sim_items(c("F", "F"), max = 5)
...
test_that("a valid factor_cor carrying scale-label dimnames is accepted", {
  # diag() of a named matrix returns a NAMED vector; attribute-blind comparison
  # must not falsely reject it (naming factor_cor by scale is natural usage).
...
test_that("a reverse-keyed item on a 0-based scale stays raw in min..max", {
  # Raw-orientation contract on a non-1 base: the output is as-clicked (the
  # reverse item loads negatively but is NOT reverse-SCORED), and the offset is
  # min - 1 + category. A no-offset / min+category mutant leaves 1..5; a
  # reflect-the-output mutant is caught by the negative-correlation slow test.
...
test_that("the bimodal trait draw is a distinct, standardised distribution (fast guard)", {
  # bimodal is brand-new (no archive precedent); the deep moment / two-camp
  # checks are slow tier. This cheap fast guard kills the cheap mutants: bimodal
  # aliasing to normal, and skipping the unit-variance standardisation. n = 2000
  # with a 0.06 band makes the kill structural, not a sampling accident -- the
  # un-standardised mixture has population sd sqrt(1.25) = 1.118 (outside 0.06)
  # and the divide-by-variance mutant has sd 1/sqrt(1.25) = 0.894 (also outside),
  # while the correct draw lands ~1 +/- 0.02. (t / skew_normal moments stay slow
  # tier by design -- TOLERANCES.md treats non-normal trait shape as a property.)
...
test_that("attentive marginals match every preset's target pmf at large n", {
  ...
  for (preset in sim_presets) {
    ...
    for (j in seq_len(ncol(x))) {
      ...
      # The DOCUMENTED contract is an absolute per-category band (TOLERANCES.md),
      # so assert max abs deviation directly -- not expect_equal(), whose waldo
      # tolerance is a (much tighter) relative metric on a near-uniform pmf.
...
test_that("a reverse-keyed item correlates negatively with its scale-mates", {
  ...
  # where a reflect-the-output mutant would invert the sign. Loadings 0.8
  # (latent r = -0.64) clears polychoric attenuation with margin; the point is
