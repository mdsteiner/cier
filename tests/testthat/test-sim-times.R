# Tests for the response-time generator + page aggregation -- the timing half of
# cier_simulate(). The kernel couples the careless lognormal shift to the planted
# careless span, so partial / temporary rows speed up only from their onset (the
# within-survey burst cier_page_time() is built to see).
#
# Trust model: oracle-only (a generator, not an index, no CRAN partner). The oracle
# (ref-sim-times.R) re-derives the cell times by hand with a coordinated draw order.
# The timing-default calibration (mu_att = log(8), mu_car = log(1.5), sigma = 0.5,
# respondent_sd = 1.2) is HELD by the slow-tier acceptance test below, not asserted:
# default careless rows must be flaggable by the cited 2 s/item page rule AND the
...
test_that("sim_timing_defaults pins the calibration", {
  expect_identical(
    sim_timing_defaults(),
...
test_that("oracle parity holds under overridden timing parameters", {
  ...
  # floored. Re-pick the seed deliberately if this fails.
...
test_that("with zero noise each cell is exactly exp(mu) on its own span side", {
  ...
  # partial row [4, 10]: the attentive prefix 1:3 stays 8; the span INCLUDES the
  # onset column (an off-by-one mutant shifts this boundary).
  expect_equal(times[3L, 1:3], rep(8, 3L), tolerance = 1e-12)
  expect_equal(times[3L, 4:10], rep(1.5, 7L), tolerance = 1e-12)
  # temporary row [3, 6]: prefix AND recovery tail attentive; both boundary columns
  # (3 and 6) are inside the careless span.
...
# =============================================================================
# SLOW TIER -- the timing-calibration acceptance test
# =============================================================================

test_that("default timing is page-flaggable and lands in the published AUC band", {
  skip_on_cran()
  skip_if_slow()
  # End-to-end at the default calibration: 4,000 respondents, half fully careless,
  # 20 five-point items on 4 scales (default one-page-per-scale rule gives 4
  # five-item pages). Acceptance:
  #   (a) total-time rank-AUC inside the published [.66, .92] band -- never 1.0
  #       (overlap is by construction; a perfectly separable simulator is wrong by
  #       spec);
  #   (b) the cited Bowling et al. (2023) 2 s/item page rule -- the shipped
  #       cier_page_time() default -- flags at least half the careless rows (the
  #       archive's mu_car = log(4) flagged essentially none);
...
test_that("partial careless rows are slower than full ones in total time", {
  skip_on_cran()
  skip_if_slow()
  # The shift applies only from the onset on, so a partial row's expected total sits
  # between the attentive and the fully-careless totals -- the coupling
