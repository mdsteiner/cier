...
# from scratch and never calls production. The numerically delicate behaviours are
# additionally pinned THROUGH the shipped indices: the Biemann footnote-2 bounce
# and cyclic diagonal via cier_lazr (2/3 and 1), the value straightliner and the
# positional scale-boundary break via cier_longstring, and the alternating period
# via cier_autocorrelation. Fast tier: deterministic pins (tol 0), index pins,
# range invariants, extent bookkeeping, the truth-schema contract, validation
# edges, seed reproducibility, and small-n recovery floors. See
...
# A raw items data.frame in the lean min/max/scale schema; `max` / `min` recycle.
...
test_that("sim_round_half_up rounds .5 boundaries UP, not to even", {
  expect_identical(sim_round_half_up(c(2.5, 3.5, 2.4, 2.6, -0.5, 0.5)),
                   c(3, 4, 2, 3, 0, 1))
  # The base-R round() (half to even) would give 2 and 4 for 2.5 / 3.5; the
  # archive midpoint ceiling((K+1)/2) and this helper agree on half-up.
...
test_that("the allowlist is the eight patterns; the registry is the seven producers", {
  ...
  # speeder is a content no-op (timing in Slice 26), so it is NOT a producer.
...
test_that("a value outside ANY item's range is a typed error", {
  # value 4 is in range for item 1 (1..5) but out for item 2 (1..3): must error,
  # not silently clip. Pins the "outside any item's range" boundary on a
  # heterogeneous battery, not just a value above every item's max.
  expect_error(
...
test_that("sequential patterns cycle on K_min when the span mixes category counts", {
  # span K = 5, 3, 5, 3 -> K_min = 3, so the cyclic walk wraps mod 3 (1,2,3,1),
  # NOT mod 5 (1,2,3,4). A max-cats or first-item-K mutant gives 1,2,3,4 and dies;
  # the K_min choice also keeps every value inside its own (possibly smaller) range.
...
test_that("markov honours the supplied transition matrix", {
  ...
  # the seed. A mutant that ignores `transition` (uses the default diag-0.6 chain)
  # produces varied later columns and dies. Deterministic: no seed needed.
...
test_that("sim_build_plan returns the Slice-25 truth schema (no speeded)", {
  ...
  expect_false("speeded" %in% names(truth))                 # Slice 26 adds it
})
...
test_that("extent uses exact proportional counts with the remainder full", {
  ...
  # deterministic; kills a mutant that drops the remainder-full bucket or
  # mis-allocates.
...
test_that("each row mutates over its OWN span, restarting the pattern at the onset", {
  ...
  # (shared-span) mutant, or one that indexes the diagonal from the global column 1
...
test_that("speeder rows leave content untouched (timing is Slice 26)", {
  items <- vit(rep("F", 6L), max = 5)
  att <- matrix(3L, nrow = 2L, ncol = 6L)
  truth <- data.frame(
...
test_that("each pattern is recovered by its matched index (rank-AUC floor)", {
  ...
  expect_gt(local({
    # random responders are multivariate outliers -> high Mahalanobis distance
    # (the canonical match; IRV's careless tail is the low-variance straightliner).
    # A coarse smoke floor: at 25% prevalence the careless rows inflate the
    # covariance, deflating their own distances, so ~0.79 here, not near 1.
