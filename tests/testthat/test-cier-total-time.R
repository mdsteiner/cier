# Tests for cier_total_time() (timing family; Ward & Meade 2023, Huang et al. 2012).
#
# Trust model: the per-respondent value is the validated seconds vector itself (an
# identity), so the oracle (ref-total-time.R) re-derives the CONTRACT -- identity
# value + the three cutoff resolutions -- by base R, never calling the wrapper. No
# CRAN parity partner (no package implements completion time as a C/IER index;
# verified 2026-06-10), so oracle + base-R quantile / median primitives are the
# parity checks, oracle-only trust like PR / RPR. Conventions under test:
# bare-numeric-vector input; strictly-positive seconds (zero / negative are input
# errors, NA abstains); empirical lower percentile (fpr 0.05) default; a third
# median-relative override frac_median in (0, 1]; the three cutoff knobs mutually
# exclusive.
...
# Five respondents, median 30 (so frac_median = 0.5 -> cutoff 15, flagging only the
# 10 s respondent) and a clear fastest / slowest.
...
test_that("cier_total_time returns the pinned cier_index schema", {
  out <- suppressWarnings(cier_total_time(small_seconds()))
  expect_cier_index_schema(out, "cier_total_time", "lower", 5L)
})

# ---- Identity oracle (tolerance 0) ------------------------------------------

test_that("the value is the seconds vector verbatim (no transform)", {
  # Regression guard for any silent sum / scale / rank / log: the value is
...
test_that("a single respondent scores, but the percentile cutoff abstains", {
  # total_time needs no pair / second item the way IRV and Laz.R do, so one finite
  # value is a valid SCORE (no n < 2 abstention on the value). The percentile CUTOFF
  # needs >= 20 scored respondents, so over a single value it abstains rather than
  # flagging the respondent against itself.
  ...
  # The median-relative override is not a tail percentile, so it still resolves on a
  # single value (the small-n floor is specific to the percentile resolver).
...
test_that("frac_median is frac * median, not (1-frac)*median or median/frac", {
  # An asymmetric frac (0.25 != 0.75) discriminates the formula: 0.25 * median(=30)
  # = 7.5, whereas (1-frac)*median = 22.5 and median/frac = 120. (The frac_median =
  # 1 test alone only rules out (1-frac) by coincidence.)
  out <- cier_total_time(small_seconds(), frac_median = 0.25)
  expect_equal(out$cutoff, 7.5, tolerance = 1e-12)
})

test_that("frac_median uses the median, not the mean", {
  # Right-skewed: mean(c(10,20,30,40,200)) = 60, median = 30. frac 0.5 -> cutoff 15
  # (median) not 30 (mean); only the 10 s row flags either way, but the CUTOFF value
...
test_that("an NA row abstains and keeps rows aligned", {
  # The abstainer sits in the middle, so value and flagged stay aligned to their
  # respondents.
  x <- c(40, NA, 120)
  out <- suppressWarnings(cier_total_time(x))
...
test_that("frac_median on an all-NA vector warns and flags nobody", {
  # The median-relative resolver mirrors the percentile abstention: no finite value
  # -> NA cutoff + the same typed warning, flagging no one.
  expect_warning(
...
test_that("zero and negative seconds are typed input errors", {
  # Total time stays strictly positive: a 0 total is an export artefact, so both
  # zero and negative error (unlike page time, which now accepts 0).
...
test_that("the zero/negative seconds error carries the export-artefact hint", {
  # The reworded hint no longer implies NA-ing a genuine fast time; it points at the
  # export instead (a 0 total is almost always a merge/export artefact).
  msg <- tryCatch(cier_total_time(c(40, 0, 120)),
                  error = function(e) gsub("\\s+", " ", rlang::cnd_message(e)))
  expect_match(msg, "export", ignore.case = TRUE)
  # The old hint ("Recode missing durations to NA") must be GONE, not supplemented
  # -- it implied destroying a genuine fast time. Asserts removal so an append-only
  # mutant cannot survive.
  expect_no_match(msg, "Recode missing durations", fixed = TRUE)
})
...
test_that("a small-but-positive time is accepted and preserved", {
  ...
  # guard that rejects genuine fast respondents.
...
test_that("frac_median = 1 is accepted (the (0, 1] upper bound)", {
  # The closed upper end: 1.0 flags the at-or-below-median half. Pins the bound as
  # (0, 1], not (0, 1).
