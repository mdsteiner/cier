# Typed condition system (R/conditions.R). Conditions are asserted by class, never message text.
...
# The `class`/`data` guards inside cier_abort/warn/inform are deliberately base-R `stop()`s (a
# typed condition cannot validate the inputs used to build it), so assert plain errors not
# `cier_error_*`.
