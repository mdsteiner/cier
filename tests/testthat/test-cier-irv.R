...
# tolerance  (matrixStats::rowSds vs
# stats::sd differ only at ulp level); the property / regression block targets
# each mutant named in population vs sample
...
test_that("cier_irv returns the pinned cier_index schema", {
  expect_cier_index_schema(suppressWarnings(cier_irv(present_fixture())),
                           "cier_irv", "lower", 4L)
})

test_that("cier_irv strips respondent row names from $value and $flagged", {
  # matrixStats::rowSds carries the input matrix's row names into the score
  # vector; the cier_index value/flagged must be BARE positional vectors -- the
  # contract cier_flagged_cases() and as.data.frame.cier_screen() rely on, and
  # which the direct/timing kernels already honour via unname(). A >= 20-row
  # continuous fixture RESOLVES the percentile cutoff, so $flagged is a real
  # named TRUE/FALSE vector without the fix -- not the unnamed all-FALSE an
  # abstaining (NA) cutoff would hand back for free, which would let a
  # "strip $value only" mutant survive.
  x <- withr::with_seed(1L, matrix(stats::runif(30L * 6L, 1, 5), nrow = 30L))
  df <- as.data.frame(x)
  ...
  expect_identical(named$value, cier_irv(x)$value)
})
...
test_that("a constant matrix abstains rather than flagging 100%", {
  # The reported bug: a constant score distribution flagged EVERY respondent at
  # any fpr. With 25 rows the small-sample guard is satisfied, so this
  # isolates the flag-everyone case: the cutoff would flag everyone, so it
  # abstains (NA + warning) and flags nobody. Values are still the exact IRV
  # scores (0); only the cutoff moves.
...
test_that("a single respondent abstains rather than flagging itself", {
  # The reported bug: n = 1 flagged the only respondent even on a perfect score.
  # A 5%-tail cutoff needs >= 20 scored respondents, so a lone respondent
...
# These snapshots also lock the flag-% format: the printed digit is the
# round(., 1)-guarded value (byte-identical to the un-guarded sprintf on the
# author's Windows R, so they do not move). The guard is platform-defensive --
# it only changes output one ulp from a rounding boundary on a divergent libc --
# so it is NOT separately verifiable on this platform; the snapshot is the lock.
