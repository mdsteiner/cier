...
# kernel); strongest_r and the abstention arithmetic are recomputed by hand.
# Reproducing the documented BFI numbers is an end-to-end anchor.
...
test_that("n_pairs is non-decreasing as critical_r falls (more pairs at a looser cutoff)", {
  # The grid runs from strictest (0.8, fewest pairs) to loosest (0.3, most), so
  # n_pairs along the rows is non-decreasing.
...
test_that("the sweep reproduces the documented BFI coverage numbers", {
  ...
  # The BFI tops out at r = 0.59, so the default 0.60 finds NO pairs (the user's bug);
  # lowering to 0.50 surfaces 23 pairs and scores 370 of 394 respondents.
...
test_that("antonym strongest_r is the MOST NEGATIVE when several antonym pairs exist", {
  # The single-pair Hadamard fixture cannot distinguish min from max on the negative
  # tail; this one has several. strongest_r must be the global minimum.
  x <- ant_matrix_c()
...
test_that("an all-constant battery yields NA strongest_r and no pairs", {
  ...
  # strongest_r is NA (not -Inf from an empty max) and every threshold finds 0 pairs.
...
test_that("the message keys on max(critical_r), not on grid position", {
  # Unsorted grid: the largest threshold (0.60) sits in the MIDDLE and finds 0 pairs.
  # A mutant keying on the first/last row (0.50 / 0.45, both find pairs) would not
  # fire; the correct condition (the strictest threshold tested) does.
  bfi <- as.matrix(bfi_careless[, 1:44])
  storage.mode(bfi) <- "double"
